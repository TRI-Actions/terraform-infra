...
WORK_DIR="/$localWorkspaceFolderBasename"
...
docker build -f "$DOCKER_FILE" -t "$DOCKER_TAG" $MAYBE_QUIET_FLAG $ARGS .
build_status=$?
...
set -e
PUID=$(id -u)
# Use user's primary group (this prevents breakage in "sudo -g docker" case)
PGID=$(id -g $(id -un))
LOGNAME=$(logname | sed 's:.*\\::')

if [ $PUID != '0' ]; then
    MOUNT="${MOUNT} --mount type=bind,source=${HOME}/.gitconfig,target=/home/${REMOTE_USER}/.gitconfig --mount type=bind,source=${HOME}/.ssh,target=/home/${REMOTE_USER}/.ssh"
    debug "MOUNT: ${MOUNT}"
else
    MOUNT="${MOUNT} --mount type=bind,source=${HOME}/.gitconfig,target=/root/.gitconfig --mount type=bind,source=${HOME}/.ssh,target=/root/.ssh"
    debug "MOUNT: ${MOUNT}"
fi

# shellcheck disable=SC2086
docker run --rm -it $DOCKER_OPTS $PORTS $ENVS $MOUNT -w "$WORK_DIR" "$DOCKER_TAG" "$SHELL" -c "
\
if [ '$REMOTE_USER' != '' ] && [ '$PUID' != '0' ] && command -v usermod &>/dev/null \
; then \
    sudo=''
    ...
    su $REMOTE_USER -s $SHELL $SHELL_ARG \
; else \
    tflint --init; \
    pre-commit install && find . -type d -name \".terraform\" -exec rm -rf {} +;
